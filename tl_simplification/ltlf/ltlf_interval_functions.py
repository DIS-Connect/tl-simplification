            ...
            match op:
                case "U":
                    if b == None or I.is_inf():
                        ...
                        I_l = IntegerSet([t for t in range(I.min(), I.max()+b+1)], False)
                    
                    if b == None or I.is_inf():
                        I_r = IntegerSet([I.min()+a], True)
                    else:
                        I_r = IntegerSet([t for t in range(I.min()+a, I.max()+b+1)], False)
                    return I_l, I_r
            ...
            match op:    
                case "G": 
                    if b == None or I.is_inf():
                        ...
                        return IntegerSet.from_interval((I.min() + a, I.max() + b), False)
                case "F":
                    if b == None or I.is_inf():
                        return IntegerSet([I.min()+a], True)
                    else:
...
@typechecked
def interval_P(I_true_r : IntegerSet, I_false_r : IntegerSet, I) -> Tuple[IntegerSet, IntegerSet]:
        a = I[0]      # The first state contained in the time interval

        # True Set:
        I_true = I_true_r.addition(a)
            
        # False Set:
        I_false = I_false_r.addition(a).union(IntegerSet.from_interval([0,a-1]))
...
def interval_U(I_true_l : IntegerSet, I_false_l : IntegerSet, I_true_r : IntegerSet, I_false_r : IntegerSet, I):
        """
        If you cant follow anything here call: +49 176 64840164
        """
